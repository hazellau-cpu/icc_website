...
function renderComponents() {
  ...
  const filtered = state.inventory.filter((item) => `${item.Component} ${item.Kit} ${item.Category} ${item.Colour} ${item.Notes}`.toLowerCase().includes(query) && (!tier || item.Tier === tier) && (!kit || item.Kit === kit));
  ...
  content.innerHTML = header('Component Inventory', 'Center-owned component stock. This inventory is not linked to any student.') + `<div class="inventory-summary"><strong>${state.inventory.length}</strong><span>Centre inventory items</span></div><div class="table-tools"><input id="componentSearch" class="table-search" placeholder="Search component, category, or kit..." value="${text(query)}"><select id="componentKit" class="table-filter"><option value="">All kits</option>${kits.map((value) => `<option ${kit === value ? 'selected' : ''}>${text(value)}</option>`).join('')}</select><select id="componentTier" class="table-filter"><option value="">All tiers</option><option ${tier === 'Tier A' ? 'selected' : ''}>Tier A</option><option ${tier === 'Tier B' ? 'selected' : ''}>Tier B</option><option ${tier === 'Tier C' ? 'selected' : ''}>Tier C</option></select><span>Page ${componentPage} / ${pageCount}</span><button class="mini-button" data-component-page="prev">←</button><button class="mini-button" data-component-page="next">→</button></div>` + table(['Component', 'Kit', 'Category', 'Tier', 'Centre qty', 'Last counted', 'Notes'], pageRows.map((item) => { const index = state.inventory.indexOf(item); return [item.Component, item.Kit, item.Category, item.Tier, `<input class="inline-input" type="number" min="0" data-center-inventory-qty="${index}" value="${escapeHtml(item['Total center qty'] || '0')}">`, item['Last counted'] || 'Not counted', item.Notes || '']; }));
  ...
  document.querySelectorAll('[data-center-inventory-qty]').forEach((input) => input.onchange = () => { const row = state.inventory[Number(input.dataset.centerInventoryQty)]; row['Total center qty'] = input.value; row['Last counted'] = today(); save('inventory', state.inventory); renderComponents(); });
}
