...
The original Excel workbook contains five sheets. CSV files cannot contain multiple sheets, so each sheet is exported as a separate CSV.
...
Use `Student ID` as the relationship key across all sheets. Do not join sheets by student name.
